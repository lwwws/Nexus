...
async function performSearch() {
    ...
    try {
        ...
        const t = state.searchMinThreadSim.toFixed(2);
        const m = state.searchMinMsgSim.toFixed(2);
        const topThreads = String(state.searchTopThreads);
...
function renderSearchResults(results, query) {
    ...
    <div style="padding-bottom: 20px;">
      <div class="sys" style="margin-bottom:10px;">
        Found ${results.length} relevant topics
      </div>
  `;

    results.forEach(group => {
        const title = group.thread_title || "Untitled Topic";
        const tscore = (typeof group.thread_score === "number") ? group.thread_score : 0;
    ...
    `;
        ...
        // Per-thread empty state (your request)
        if (msgs.length === 0) {
            html += `<div class="empty">No relevant messages found for "${esc(query)}".</div>`;
        } else {
            msgs.forEach(m => {
                const time = formatTime(m.timestamp);
                const nameCol = userColor(m.user);
                const mscore = (typeof m.score === "number") ? m.score : 0;
          ...
          <div class="msgRow left" style="cursor:pointer;"
            ...
            <div class="bubble">
              <div class="metaLine">
                <span class="name" style="color:${esc(nameCol)}">${esc(m.user)}</span>
                <span class="time">${esc(time)} <span style="opacity:0.5">(${(mscore * 100).toFixed(0)}%)</span></span>
              </div>
              <div class="text">${esc(m.text)}</div>
            </div>
          </div>
